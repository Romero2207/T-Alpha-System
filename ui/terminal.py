...
# МЕНЮ ОТКРЫТО ПО УМОЛЧАНИЮ (expanded)
...
# --- БЛОК УПРАВЛЕНИЯ РИСКОМ С ЗАЩИТОЙ ОТ КРАШЕЙ ---
...
# --- НЕУБИВАЕМЫЙ ГРАФИК TRADINGVIEW (IFRAME) ---
...
with tab_terminal:
    ...
    change_pct = ((current_price - df_chart['open'].iloc[0]) / df_chart['open'].iloc[
        0]) * 100 if not df_chart.empty else 0.0
        ...
        if chart_type == "TradingView Pro (Рисование)":
            ...
            if not df_chart.empty:
                ...
                fig.add_trace(go.Bar(x=df_chart['timestamp'], y=df_chart['volume'], marker_color=colors, name='Объем'),
                              row=2, col=1)
                fig.update_layout(height=600, margin=dict(l=0, r=50, t=10, b=0), plot_bgcolor='#0B0E14',
                                  paper_bgcolor='#0B0E14', xaxis_rangeslider_visible=False, showlegend=False,
                                  dragmode='pan')
